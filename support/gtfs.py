...
def fillTrips(
    ...
    restrictService: Iterable[str] | set[str] = {},
) -> tuple[dict[Hashable, TripsEntry], set[Hashable]]:
    """
    ...
    with open(filename, mode="r", newline="") as inFile:
        ...
        for fileLine in csvReader:
            ...
            if (
                shapeID in unusedShapeIDs
                or len(restrictService) > 0
                and serviceID not in restrictService
            ):
                ...
                if shapeID not in shapes:
                    if shapeID not in shapeErrorIDs:
                        logging.warning(
                            f"GTFS Trip {tripID} expects undefined shape ID {shapeID}; skipping"
                        )
                    ...
                    if routeID not in routes:
                        logging.warning(
                            f"GTFS Trip {tripID} expects undefined route ID {routeID}; skipping"
                        )
                        unusedTripIDs.add(tripID)
                    else:
                        newEntry = TripsEntry(
...
def fillStopTimes(
    ...
    unusedTripIDs: Iterable[Hashable],
) -> dict[TripsEntry, list[StopTimesEntry]]:
    """
    ...
    with open(filename, mode="r", newline="") as inFile:
        ...
        for fileLine in csvReader:
            ...
            if tripID not in unusedTripIDs:
                ...
                if not stopID in stops:
                    logging.warning(
                        f"GTFS Stop Times file expects undefined stop ID {stopID}"
                    )
                    continue
                newEntry = StopTimesEntry(
                    trip=trips[tripID],
                    stop=stops[stopID],
        ...
        if badTrips:
            ...
            for tripID in sorted(badTrips):
                ...
                strOut += str(tripID)
            logging.warning(
                f"GTFS Stop Times file expects undefined trip ID(s) {strOut}"
            )
