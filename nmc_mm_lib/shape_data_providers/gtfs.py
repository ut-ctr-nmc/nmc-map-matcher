...
from nmc_mm_lib import shape_data_provider
from nmc_mm_lib import track
import os, sys, csv
from datetime import datetime, timedelta

class GTFS(shape_data_provider.ShapeDataProvider):
    """
    Represents a GTFS file set that represents track data and associated transit data.
    ...
    def provideCmdLineOpts(self, options):
        ...
        options.add_argument("-gp", nargs=1, type=str, metavar="GTFSPATH", dest="gtfsPath",
            help="If using GTFS for track/transit input, specifies the path to the GTFS set")
        options.add_argument("-gs", nargs=1, type=str, metavar="GTFSSHAPES", dest="gtfsShapes",
            help="If using GTFS for track/transit input, specifies an alternate GTFS shapes filename")
    ...
    def readShapesEntries(self, suppressOutOfOrder=False):
        """
        readShapesEntries returns a generator that gives out track entries one by one.
        At each call, the next valid constructed track.ShapesEntry is yielded. You must check the
        shapeID to verify whether you are in the same track as the previous call.
        @param suppressOutOfOrder: Set this to False to check that all elements come back in
            sequence order, grouped together per track ID, otherwise that isn't guaranteed.
        """
        loadedShapes = {}
        "@type loadedShapes: dict<str, track.Shape>"
        with open(self.shapeFilename, 'r') as inFile:
            ...
            for fileLine in csvReader:
                ...
                if fileLine["shape_id"] not in loadedShapes:
                    loadedShapes[fileLine["shape_id"]] = track.Shape(fileLine["shape_id"])
                newEntry = track.ShapesEntry(loadedShapes[fileLine["shape_id"]], int(fileLine["shape_pt_sequence"]),
    ...
    def readRoutes(self, shapes):
        ...
        with open(filename, 'r') as inFile:
            ...
            for fileLine in csvReader:
                ...
                newEntry = track.Route(fileLine["route_id"], fileLine["route_short_name"], fileLine["route_long_name"])
    ...
    def readTrips(self, shapes, routes, unusedShapeIDs=set(), restrictService=set()):
        ...
        @type shapes: track.Shapes
        @type routes: track.Routes
        @type unusedShapeIDs: set<str>
        @type restrictService: set<str>
        @return A dict of tripID to track.Trip records, as well as a list of unused trip IDs
        @rtype (track.Trips, set<str>)
        """
        ret = track.Trips()
        unusedTripIDs = set()
            ...
            for fileLine in csvReader:
                ...
                if (shapeID in unusedShapeIDs) or ((len(restrictService) > 0) and (serviceID not in restrictService)):
                    ...
                    if shapeID not in shapes:
                        print("WARNING: GTFS Trips file expects undefined track ID " + shapeID, file=sys.stderr)
                        unusedTripIDs.add(tripID)                        
                    else:
                        if routeID not in routes:
                            ...
                            newEntry = track.Trip(tripID, routes[routeID], fileLine["trip_headsign"], shapes[shapeID])
    ...
    def readStops(self):
        ...
        with open(filename, 'r') as inFile:
            ...
            for fileLine in csvReader:
                ...
                newEntry = track.Stop(fileLine["stop_id"], fileLine["stop_name"], float(fileLine["stop_lat"]), float(fileLine["stop_lon"]))
            ...
            for fileLine in csvReader:
                ...
                if fileLine["tripID"] not in unusedTripIDs:
                    ...
                    if not fileLine["stop_id"] in stops:
                        ...
                        continue
                    newEntry = track.StopTime(fileLine["tripID"], fileLine["stop_id"], int(fileLine["stop_sequence"]))
